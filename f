...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = Shared.Forbidden.PointAssist.Enabled -- Visibility based on the enabled setting
            Circle.Radius = Shared.Forbidden.PointAssist.Reach -- Reach from your table
...
local function GetVelocity(player, part)
    if player and player.Character then
        local velocity = player.Character[part].Velocity
        if velocity.Y < -30 and Shared.Forbidden.Silent.Aligner then
            Shared.Forbidden.Silent.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and Shared.Forbidden.Silent.Aligner then
